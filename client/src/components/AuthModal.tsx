...
export const AuthModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl rounded-2xl border border-cyan-500/40 bg-[#0b0f19] p-6 shadow-2xl shadow-cyan-950/50 flex flex-col space-y-5 overflow-hidden">
                  ...
                  <button
                    ...
                    className={`p-3.5 rounded-xl border text-left transition-all cursor-pointer flex flex-col justify-between group relative overflow-hidden ${
                      isCurrent
                        ...
                        : 'border-slate-800 bg-slate-900/60 hover:border-cyan-500/60 hover:bg-slate-800/80'
                    }`}
                  >
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      <span className={`text-[10px] font-mono px-2 py-0.5 rounded border ${roleColors[u.role] || 'text-cyan-400 border-cyan-800'}`}>
                        {u.role.replace('_', ' ')}
                      </span>
                    </div>
          ...
          <form onSubmit={handleLoginSubmit} className="space-y-4 relative z-10">
            <div className="space-y-1">
              <label className="text-xs font-mono text-slate-300 flex items-center gap-1.5">
                <Mail className="w-3.5 h-3.5 text-cyan-400" /> Operative Email
              </label>
              <input
                type="email"
                required
                ...
                className="w-full px-3.5 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-white font-mono text-sm focus:border-cyan-400 focus:outline-none"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-mono font-bold text-sm transition-colors cursor-pointer shadow-lg shadow-cyan-500/20 flex items-center justify-center gap-2"
            >
              {loading ? (
        ...
        {activeTab === 'REGISTER' && (
          <form onSubmit={handleRegisterSubmit} className="space-y-3 relative z-10">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-700 text-white font-mono text-sm focus:border-cyan-400 focus:outline-none"
                />
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                <label className="text-xs font-mono text-slate-300 flex items-center gap-1.5">
                  <BadgeAlert className="w-3.5 h-3.5 text-cyan-400" /> Tactical Role
                </label>
                <select
                  value={regRole}
